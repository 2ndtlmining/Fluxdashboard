...
export class DashboardComponent implements OnInit {
  ...
  Rate: any = 0;
  data: any = [];
  ipValues: any = JSON.parse(JSON.stringify(contants))
    .IPValues.split(',')
  ...
  AssetUnderManagement() {
    if (this.OverviewData != undefined && this.Rate > 0) {
      return (
        (this.getCUMULUS() *
          JSON.parse(JSON.stringify(contants)).CumulusConstant +
          this.getNIMBUS() *
            JSON.parse(JSON.stringify(contants)).NimbusConstant +
          this.getSTRATUS() *
            JSON.parse(JSON.stringify(contants)).StratusConstant) *
        this.Rate
      );
    }
    return 0;
  ...
  getPaymentAddressCount() {
    if (this.OverviewData != undefined) {
      const PaymentAddress = this.OverviewData.map(
        (item: { payment_address: any }) => item.payment_address
      );
      return this.getDistinctElements(PaymentAddress).length;
    }
    return 0;
  }
}
